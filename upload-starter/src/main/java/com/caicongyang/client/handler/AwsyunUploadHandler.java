 ...
 */
public class AwsyunUploadHandler extends UploadHandler  {


    private static Logger logger = LoggerFactory.getLogger(AwsyunUploadHandler.class);
     ...
     * 也可以把 obsClient 的 static 去掉
     */
    public AwsyunUploadHandler() {
        if ( s3 != null){
            return ;
        }
        AmazonS3ClientBuilder amazonS3ClientBuilder = AmazonS3ClientBuilder.standard();

        amazonS3ClientBuilder.setCredentials( new AWSStaticCredentialsProvider( new BasicAWSCredentials(UploadConfig.FILE_AK,UploadConfig.FILE_SK)));

        String[] parts=StringUtils.split(UploadConfig.END_POINT,".");
        if(parts==null || parts.length<2){
            throw new IllegalArgumentException("endpoint无效:"+UploadConfig.END_POINT);
        }
        amazonS3ClientBuilder.setEndpointConfiguration( new AwsClientBuilder.EndpointConfiguration(UploadConfig.END_POINT,parts[1]));
    ...
    public UploadResult delete(List<String> fileList, String poolName, boolean deleteSeriesImage) throws UploadException {
        ...
        for (String fileUrl : fileList){
            ...
            try {
                ...
                s3.deleteObject(UploadConfig.BUCKET_NAME,storePathInKsyun);
                successCount++;
